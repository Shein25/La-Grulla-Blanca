# Grulla Blanca — Fase I · retest Piel G234_D1 + Filamento · Monte Carlo reproducible
...
#
# Política MULTI_READER (stress-test táctico, sin RNG futuro):
...
PIEL_R1=[{"guard":6},{"durationExtra":1},{"cost":-1}]
PIEL_R2=[{"cost":-1},{"guard":7},{"durationExtra":1}]
...
def piel_conf(cfg):
    p1,p2=cfg; cost=5; guard=3; duration=2
...
def simulate(root,o1,o2,techs,choices,boss_def,runs,seed=1):
    ...
    for rnd in range(1,81):
        ...
        if cycle==0:
            # Golpe 1: precarga Piel si existe y la burbuja ya no está.
            act[:]=1
            if cc is not None:
                need=(piel_capacity[idx]<=0)&(qi[idx]>=cc["cost"])
                act[need]=5
        elif cycle==1:
            # Golpe 2: precarga Paso si hace falta para que siga activo en Campanada.
            act[:]=1
            if pc is not None:
...
def main():
    ...
    ap.add_argument("--loadout",choices=("piel_filamento",),default="piel_filamento")
