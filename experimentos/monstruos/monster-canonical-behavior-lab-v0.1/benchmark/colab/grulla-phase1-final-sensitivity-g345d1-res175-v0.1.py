...
# - Piel G345_D1 óptima + Resonancia x1.75: Cobre flexible + Cobre grueso
#
# Uso:
...
STRATEGIES={0:"READER",1:"PASO",2:"FILAMENTO",3:"PIEL_G345_RES175"}
...
def one_duel(strategy,root,o1,o2,boss_def,pata_bonus):
    ...
    # Piel G345_D1 óptima: coste 5, Guardia 5, duración 2, reserva 10.
    piel_cost=5; piel_guard_max=5; piel_cap_max=10
    ...
    burn_turns=0; deb=0; deb_turns=0; def_next=0; resonance_ready=0
    ...
    for rnd in range(1,81):
        ...
        if bound>0:
            ...
            if cyc==3:
                def_next=pata_bonus
                if strategy==3: resonance_ready=1
            else:
                ...
                if hit:
                    ...
                    if r==20: dmg=math.ceil(dmg*1.5)
                if strategy==3 and cyc==0 and resonance_ready and dmg>0 and piel_cap>0:
                    dmg=math.ceil(dmg*1.75)
                ...
                hp-=dmg
                if strategy==3 and cyc==0 and resonance_ready:
                    resonance_ready=0
...
def main():
    ...
    df.to_csv("grulla_phase1_final_sensitivity_g345_res175_v0.1.csv",index=False)
    summary.to_csv("grulla_phase1_final_sensitivity_g345_res175_summary_v0.1.csv",index=False)
