...
#
# Perfil fijo de control: ATQ 4, DEF 13, Golpe 1d6+2.
#
# Sensibilidad de FX:
# HIST      = valores históricos provisionales
# TACT_A    = Tormenta +2 2d6+2 drain2; Cerrar 3/6; Recordar +15; Eco condicional 4
# TACT_B    = Tormenta +2 2d6+2 drain1; Cerrar 3/6; Recordar +15; Eco condicional 3
# TACT_C    = Tormenta +1 2d6+2 drain1; Cerrar 3/6; Recordar +10; Eco condicional 3
# TACT_D    = Tormenta +2 1d8+3 drain1; Cerrar 3/6; Recordar +15; Eco condicional 3
#
# En TACT_* Eco sólo drena si la acción del jugador gastó qi, respetando la matriz
# de counterplay: BASIC/DEFEND durante la ventana son respuestas universales.
#
# Estrategias:
...
PROFILES={0:("SAME",4,13,2)}
FX_NAMES={0:"HIST",1:"TACT_A",2:"TACT_B",3:"TACT_C",4:"TACT_D"}
...
@njit
def duel(fxmodel,profile,strategy,root,o1,o2,seed):
    np.random.seed(seed)
    ratt,rdef,tcost,nd,fd,tflat,tab,tcrit,add,rburn,rdeb,rdebd=root_stats(root,o1,o2)
    # profile mapping
    bat=4; bdef=13; bflat=2
    if fxmodel==0:
        storm_ab=3; storm_n=2; storm_f=6; storm_flat=3; storm_drain=2
        close_guard=4; close_cap=8; remember_eva=20; eco_drain=4; eco_conditional=0
    elif fxmodel==1:
        storm_ab=2; storm_n=2; storm_f=6; storm_flat=2; storm_drain=2
        close_guard=3; close_cap=6; remember_eva=15; eco_drain=4; eco_conditional=1
    elif fxmodel==2:
        storm_ab=2; storm_n=2; storm_f=6; storm_flat=2; storm_drain=1
        close_guard=3; close_cap=6; remember_eva=15; eco_drain=3; eco_conditional=1
    elif fxmodel==3:
        storm_ab=1; storm_n=2; storm_f=6; storm_flat=2; storm_drain=1
        close_guard=3; close_cap=6; remember_eva=10; eco_drain=3; eco_conditional=1
    else:
    ...
    for rnd in range(1,81):
        ...
        if act==4:
            ...
            if hit:
                ...
                if reserve>0:
                    a=min(dmg,close_guard,reserve); dmg-=a; reserve-=a
                bhp-=dmg
        ...
        if intent==GOLPE:
            ...
            if (not eco_conditional) or qspent>0:
                qi=max(0,qi-eco_drain)
...
@njit
def scenario(runs,seed,fxmodel,profile,strategy,root,o1,o2):
    np.random.seed(seed)
    wins=0; rounds=0; hp=0; qi=0; locks=0
    for i in range(runs):
        # duel reseedea para reproducibilidad por duelo
        z=duel(fxmodel,profile,strategy,root,o1,o2,seed+i*7919+17)
...
def main():
    ...
    args=ap.parse_args()
    duel(0,0,2,0,1,1,1) # JIT warmup

    rows=[]
    profile=0
    for fxmodel in range(5):
      for strategy in range(4):
       for root in range(3):
        for o1 in range(1,4):
         for o2 in range(1,4):
          seed=(fxmodel+1)*100000000+(strategy+1)*10000000+(root+1)*1000000+o1*10000+o2*100
          w,rr,h,q,l=scenario(args.runs,seed,fxmodel,profile,strategy,root,o1,o2)
          rows.append(dict(fx=FX_NAMES[fxmodel],profile=PROFILES[profile][0],strategy=STRATEGIES[strategy],root=root,o1=o1,o2=o2,
                           winRate=w,avgRounds=rr,avgHp=h,avgQi=q,lockEndRate=l))
    df=pd.DataFrame(rows)
    summary=df.groupby(["fx","strategy","root"]).winRate.agg(["mean","min","max"]).reset_index()
    global_summary=df.groupby(["fx","strategy"]).winRate.agg(["mean","min","max"]).reset_index()
    print("\n=== GLOBAL ===")
    ...
    df.to_csv("grulla_phase2_fx_sensitivity_v0.1.csv",index=False)
    summary.to_csv("grulla_phase2_fx_sensitivity_roots_v0.1.csv",index=False)
    global_summary.to_csv("grulla_phase2_fx_sensitivity_global_v0.1.csv",index=False)
